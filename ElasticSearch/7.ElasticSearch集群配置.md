...
```
cluster.name:my-es --集群名称，一个集群的节点需要设置相同名称
node.name:node-1  -- 节点名称
network.host:192.168.25.250 -- 本机ip，保存unicast可以发现，进入非回环
http.port:9200 -- #服务端口号，默认9200
transport.tcp.port:9300 -- #集群间通信端口号， 默认9300
discovery.zen.ping.unicast.hosts: ["es01","es02","es03"] -- 集群自动发现的机器ip集合
```
